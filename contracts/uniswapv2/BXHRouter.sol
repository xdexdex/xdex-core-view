...
contract DDXRouter is IUniswapV2Router02,Ownable {
    ...
    function addLiquidityHT(
        ...
        if (msg.value > amountHT) TransferHelper.safeTransferNative(msg.sender, msg.value - amountHT);
    }
    ...
    function removeLiquidityHT(
        ...
        TransferHelper.safeTransferNative(to, amountHT);
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = IUniswapV2Factory(factory).getAmountsOut(pair, amountIn, path);
    ...
    function swapTokensForExactTokens(
        ...
        address pair = UniswapV2Library.pairFor(factory, path[0], path[1]);
        amounts = IUniswapV2Factory(factory).getAmountsIn(pair, amountOut, path);
        ...
        returns (uint[] memory amounts)
    {
        ...
        address pair = UniswapV2Library.pairFor(factory, path[0], path[1]);
        amounts = IUniswapV2Factory(factory).getAmountsOut(pair, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWHT(WHT).deposit{value: amounts[0]}();
        assert(IWHT(WHT).transfer(pair, amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = IUniswapV2Factory(factory).getAmountOut(address(pair),amountInput, reserveInput, reserveOutput);
            }
            if(swapMining!=address(0x0)){
