...
contract DDXPool is Ownable {
    ...
    function safeDDXTransfer(address _to, uint256 _amount) internal {
        ...
        (uint256 upperFee,uint256 upperFeeTotal) = IUniswapV2Factory(factory).getReferAmount(_to,_amount);
        if(upperFee>0){
            ddx.mint(address(factory), upperFee);
            IUniswapV2Factory(factory).rewardUpper(_to,address(ddx),upperFee,upperFeeTotal);         
        }
